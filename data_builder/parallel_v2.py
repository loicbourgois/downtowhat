...
async def parallel_v2_async(
    ...
    def clean(i, _):
        ...
        if verbose:
            ...
            logging.info(f"{spaces}[ end ] {function_async.__name__}() {len(done)}/{len(data)} - ETA {eta}")
    task_wrappers = []
    for i, x in enumerate(data):
        ...
        while True:
            ...
            if len(queue) < concurrency:
                break
        if verbose:
            logging.info(f"{spaces}[start] {function_async.__name__}() {i+1}/{len(data)}")
        queue.add(i)
        task_wrapper['task'] = asyncio.create_task( function_async(task_wrapper['input']) )
...
def parallel_v2(data, function_async, **kwargs):
    if kwargs.get('limit') is not None:
        return asyncio.run(parallel_v2_async(data[:kwargs['limit']], function_async, **kwargs))
