...
def transform(row):
    for i, k1 in enumerate(option_keys[:-1]):
        for k2 in option_keys[i+1:]:
            if type(row[k1]) == str and type(row[k2]) == str and len(row[k1]) and len(row[k2]):
...
prompt = read(f"{os.environ['HOME']}/github.com/loicbourgois/downtowhat/data_builder/prompt.md")
data = []
for i in range(1):
    for persona in [
            'Hitler',
