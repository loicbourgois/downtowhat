...
def cleanup(df):
    def transform(row):
        try:
            row['qid'] = hashlib.sha256(str(f"{row['question']} | {row['a']} | {row['b']}").encode('utf-8')).hexdigest()
            ...
            if row['a'].lower() in answer.lower() and row['b'].lower() not in answer.lower():
                row['answer'] = row['a']
            elif row['a'].lower() not in answer.lower() and row['b'].lower() in answer.lower():
                row['answer'] = row['b']
...
def compute_similarity(df):
    ...
    for p1 in personas:
        for p2 in personas:
            if p1 != p2:
                similarity_count = pandasql.sqldf(f"""
                    with q1 as (
                        select * from df
                        where df.persona = '{p1}'
                    ),
                    ...
                    on q1.qid = q2.qid
                        ...
                        and q1.answer != ''
                """, locals()).similarity_count[0]
                question_count = pandasql.sqldf(f"""
                    with q1 as (
                    ...
                    from q1
                """, locals()).question_count[0]
                question_count_1 = pandasql.sqldf(f"""
                    with q1 as (
                        select distinct qid from df
                        where persona = '{p1}'
                            and answer != ''
                    ...
                    from q1
                """, locals()).question_count_1[0]
                question_count_2 = pandasql.sqldf(f"""
                    with q1 as (
                        select distinct qid from df
                        where persona = '{p1}'
                            and answer != ''
...
paths = glob.glob(f"{os.environ['HOME']}/github.com/loicbourgois/downtowhat/data_builder/archive/*.csv")
...
with pandas.option_context('display.max_columns', 400, "display.max_rows", None, 'display.width', 400):
